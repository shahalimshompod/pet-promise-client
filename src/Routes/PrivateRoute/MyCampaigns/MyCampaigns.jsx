...
const MyCampaigns = () => {
    ...
    // query function
    const fetchMyCampaignData = async ({ queryKey }) => {
        const [, page] = queryKey;
        const res = await axiosSecure.get(`/my-campaign-data?email=${currentUserEmail}&page=${page}&limit=${limit}`)

        return res?.data;
    }

    const { data, isLoading, isError, refetch } = useQuery({
        queryKey: ["my-campaign-data", page],
        queryFn: fetchMyCampaignData,
    ...
    }, [selectedCampaignForDonators, selectedCampaign]);

    


    return (
        <div className="bg-cyan-100 dark:bg-[#111827] lg:w-full rounded-xl border-2 border-cyan-500 py-6 px-3 lg:px-6">
                                ...
                                >
                                    ...
                                    <td className="px-6 py-4">
                                        <div className='flex items-center justify-evenly mb-1'>
                                            ...
                                            <span className={`nunito ${item.expired ? 'bg-red-500' : item.isPaused ? 'bg-red-500' : 'bg-green-500'} text-cyan-50 rounded px-4`}>
                                                {item.expired ? 'Expired' : item.isPaused ? 'Paused' : 'Active'}
                                            </span>
                                        </div>
                                        {/* Progress Bar */}
                                    ...
                                    <td className="px-6 py-4">
                                        <div className='flex items-center justify-around'>
                                            ...
                                            <button
                                                ...
                                                className={` ${item.isPaused ? 'text-red-500 hover:text-red-600' : 'text-green-500 hover:text-green-400'}`}>
