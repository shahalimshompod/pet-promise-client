...
const DonationDetails = () => {
    ...
    const { user, signInWithGoogle, signInWithFacebook, setUser } = useContext(AuthContext)
    ...
    const [selectedCampaignToDonate, setSelectedCampaignToDonate] = useState(null)
    ...
    const calculateTimeRemaining = (lastDate) => {
        ...
        const minutesRemaining = differenceInMinutes(new Date(lastDate), today) % 60;  // Get remaining minutes after calculating hours
    ...
    const { data: recommendedData, isLoading: recLoading, isError: recError, refetch: recRefetch } = useQuery({
        queryKey: ['recommendedData'],
        queryFn: fetchRecommendedDonation
    })

    if (isLoading) {
        return (<div className="text-center my-8 flex flex-col items-center h-screen justify-center dark:bg-[#111827] bg-cyan-50">
        ...
        <div className='w-full mx-auto bg-gradient-to-br from-cyan-100 via-cyan-50 to-cyan-100 dark:bg-gradient-to-br dark:from-[#111827]/90 dark:via-[#111827]/70 dark:to-[#111827]/90 shadow-lg overflow-hidden'>
            ...
            <div className="container mx-auto px-4 lg:px-5">
                <div className="my-10 flex flex-col lg:flex-row justify-between gap-10 lg:gap-16">
                    <div className="flex flex-col lg:flex-row gap-8 lg:gap-16">
                        <div className="flex flex-col lg:flex-row gap-10">
                            <div className="w-full lg:w-[300px] lg:h-[300px] overflow-hidden rounded-lg">
                                <img className="object-cover w-[300px] h-[300px]" src={data?.petPicture} alt="pet" />
                            </div>
                            <div>
                                <h1 className="text-2xl lg:text-3xl font-bold poppins dark:text-white/80 text-black/80">{data?.petName}</h1>
                                <p className="text-sm lg:text-base text-gray-600 dark:text-gray-400">
                                    Started: {formatDate(data?.campaignAddedDate).date} | {formatDate(data?.campaignAddedDate).time}
                                </p>
                                <h1 className="text-lg lg:text-base gap-1 font-semibold mt-4 dark:text-white/80 flex items-center">
                                    <FaClock /> <span className={`font-bold ${data?.donatedAmount === data?.maxDonation ? 'text-black dark:text-white/80' : 'text-green-500 dark:text-green-300'} `}>
                                        {
                                            data?.donatedAmount === data?.maxDonation ? 'N/A' : `${daysRemaining.days} Days ${daysRemaining.hours} hrs ${daysRemaining.minutes} mins`
                                        }
                                    </span>
                                </h1>

                                {/* status */}
                                <p className='nunito my-3 flex items-center gap-3 text-black/80 dark:text-cyan-50'><p>Status:</p> <p className={`font-medium nunito px-3 rounded text-white ${data.isPaused ? "bg-red-500" : "bg-green-500"}`}>
                                    {
                                        data?.isPaused ? <span>
                                            {
                                            ...
                                            {
                                                data?.donatedAmount === data?.maxDonation ? <span className='flex items-center gap-2'><span>Goal Achieved</span><span><BsFillPatchCheckFill /></span></span> : "Active"
                                            }
                                        </span>
                                    }
                                </p></p>
                                ...
                                <div className="mt-8">
                                    <h1 className="text-xl lg:text-2xl font-bold text-green-500 dark:text-green-300">
                                        ${data?.donatedAmount} RAISED OF <span className="text-lg">${data?.maxDonation}</span>
                                    </h1>
                                    {/* Progress Bar */}
                        ...
                        <div className="flex flex-col items-center gap-4 lg:gap-6">
                            <div className="bg-cyan-600 dark:bg-[#111827] w-48 py-4 rounded-xl flex flex-col items-center">
                                <h1 className="text-white text-2xl lg:text-4xl font-bold">${data?.maxDonation}</h1>
                            ...
                            </div>
                            {
                                user?.email === data?.currentUserEmail ? (
                                    <Link to='/dashboard/my-donation-campaigns'>
                                        <button
                                            type="button"
                                            className={`text-white bg-cyan-500 dark:bg-[#111827] hover:bg-cyan-700 font-medium rounded-lg px-5 py-2.5 w-full text-sm lg:text-xl nunito ${data?.donatedAmount == data?.maxDonation && 'hidden'}`}
                                        >
                                    ...
                                    <button
                                        ...
                                        type="button"
                                        className={`text-white bg-cyan-500 dark:bg-[#111827] hover:bg-cyan-700 font-medium rounded-lg px-5 py-2.5 w-full text-lg lg:text-xl nunito ${data?.donatedAmount == data?.maxDonation && 'hidden'}`}
                                    >
                                        DONATE NOW
                                    </button>
                                )
                    ...
                    <div className='h-[500px] w-full overflow-y-scroll scrollbar-none'>
                        <div className="mt-8">
                            <h1 className="text-xl lg:text-2xl font-semibold poppins dark:text-white/80 text-black/80">Short Description about this campaign</h1>
                        ...
                        <div className="mt-8">
                            <h1 className="text-xl lg:text-2xl font-semibold poppins dark:text-white/80 text-black/80">About Campaign</h1>
                            <div
                                className="text-sm lg:text-base text-gray-700 dark:text-gray-300 roboto mt-2"
                                dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(data?.longDescription) }}
                            />
                        </div>
                    </div>
                </div>
            ...
            <div className='container mx-auto'>
                <h1 className='text-4xl poppins font-semibold uppercase mb-6 text-black/80 dark:text-white/80'>Recommended Donation Campaigns</h1>
                <div className='flex flex-col items-center'>
                    <div className='grid grid-cols-1 lg:grid-cols-3 mb-16 gap-10 '>
                        {recLoading && <CampaignCardSkeleton cards={3} />}
                        {
                            recommendedData?.map((data, idx) => <CampaignCard key={idx} recRefetch={recRefetch} refetch={refetch} data={data} />)
                        }
                    </div>
                </div>
            </div>
            {
                user ? isOpened && <Payment refetch={refetch} isOpen={openModal} onClose={closeModal} selectedData={selectedCampaignToDonate} /> : <LoginModal petForAdopt={selectedCampaignToDonate} setPetForAdopt={setSelectedCampaignToDonate} signInWithGoogle={signInWithGoogle} signInWithFacebook={signInWithFacebook} setUser={setUser} />
            }
        </div>
    );
};
